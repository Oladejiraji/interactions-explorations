...
function FlowerMenuItem(props: IFlowerMenuItem) {
  ...
  return (
    <motion.button
      className="w-16 h-16 rounded-full absolute top-1/2 left-1/2 flex items-center justify-center cursor-pointer border border-white/10"
      initial={{
        x: -halfPetal,
        y: -halfPetal,
        opacity: 1,
      ...
      onHoverEnd={(e) => {
        (e.target as HTMLElement).style.boxShadow =
          "0 4px 12px rgba(0,0,0,0.5), inset 0 1px 1px rgba(255,255,255,0.08)";
      }}
      whileTap={{ scale: 0.95 }}
      transition={{
        duration: 0.4,
      ...
      aria-label={label}
    >
      <Icon size={24} color="#a0a0a8" strokeWidth={1.8} />
      <motion.span
        className="absolute -bottom-7 text-xs font-medium text-white/40 whitespace-nowrap pointer-events-none"
  ...
  return (
    <div className="w-full h-screen flex items-center justify-center bg-[#0e0e10]">
      ...
      >
        ...
        <motion.button
          type="button"
          className="w-14 h-14 rounded-full relative z-10 flex items-center justify-center cursor-pointer border border-white/10"
          style={{
            background: "linear-gradient(145deg, #2a2a2e, #1a1a1e)",
            boxShadow:
              "0 4px 16px rgba(0,0,0,0.6), inset 0 1px 1px rgba(255,255,255,0.08)",
          }}
        ...
        >
          <Plus size={28} color="#a0a0a8" strokeWidth={2} />
        </motion.button>
      </motion.div>
    </div>
  );
